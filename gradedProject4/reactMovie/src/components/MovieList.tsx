...
const MovieList = (props: Props) => {
  ...
  const handleFavourites = async (movie: IDataList) => {
    try {
      if (section === "favourite") {
        ...
        const itemsAfterDelete = receivedData.filter(
          (data) => data.id !== movie.id
        );
        setToast([
          ...toast,
          {
            toastString: `${movie.title} removed from Favourites`,
            toastBgType: "success",
          },
        ]);
        ...
        const movieAlreadyPresent = favouriteMovies.filter(
        ...
        if (movieAlreadyPresent.length === 0) {
          ...
          setToast([
            ...toast,
            {
              toastString: `${movie.title} added to Favourites`,
              toastBgType: "info",
            },
          ]);
        } else {
          setToast([
            ...toast,
            {
              toastString: `${movie.title} already present in Favourites`,
              toastBgType: "warning",
            },
          ]);
        }
  ...
  const favouriteText =
    section === "favourite" ? "Remove from favourites" : "Add to favourites";
  ...
  return (
    <>
      <Container>
        ...
        <ToastContainer
          className="p-3"
          style={{ zIndex: 2, position: "fixed", right: "20px", top: "20px" }}
        >
          {toast.map((toast, idx) => (
            <ToastItem
              key={idx}
              toastString={toast.toastString}
              toastBgType={toast.toastBgType}
            />
          ))}
        </ToastContainer>
        {filteredItems.length > 0 ? (
          ...
          >
            {section === "favourite"
              ? "Favourites is empty. Please add movies."
              : "No data."}
          </div>
        )}
      </Container>
