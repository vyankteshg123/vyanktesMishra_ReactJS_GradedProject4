...
const getDataFromServer = (section: string) => {
  ...
  return axios.get<IDataList[]>(url).then((response) => response.data);
};

const addToFavourites = (movieData: Omit<IDataList, "id">) => {
  return axios
    .post<IDataList>(`http://localhost:3001/favourite`, movieData, {
      headers: {
...
const removeFromFavourites = (id: string | number) => {
  return axios
    .delete<IDataList>(`http://localhost:3001/favourite/${id}`, {
      headers: {
        "Content-Type": "application/json",
      },
    })
